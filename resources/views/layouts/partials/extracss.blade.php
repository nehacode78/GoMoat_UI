  ...
  .sidebar-collapse .app-footer-fixed {
      left: 4rem;
      width: calc(100% - 4rem);
  }
...
/* Ball EXACTLY centered on the line */
.side-bar .submenu-link::before {
    content: '';
    position: absolute;
    left: -1.25rem;               /* SAME as line position */
    top: 50%;
    transform: translate(-96%, -75%); /* CENTER on line */
    width: 6px;
    height: 6px;
    border-radius: 50%;
    background-color: rgba(255, 255, 255, 0.45); /* SAME AS LINE */
}

/* Active submenu */
.side-bar .submenu-link.active {
    color: #ffffff;
}

/* Active ball */
.side-bar .submenu-link.active::before {
    background-color: #ffffff;
}

/* Hover effect */
.side-bar .submenu-link:hover::before {
    background-color: #ffffff;
}
