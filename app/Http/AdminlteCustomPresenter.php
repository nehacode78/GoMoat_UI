...
public function getOpenTagWrapper()
{
    ...
    {
        return '<a href="' . $item->getUrl() . '" title="" class="tw-flex tw-items-center tw-gap-3 tw-px-3 tw-py-2 tw-text-sm  tw-font-medium tw-tracking-tight tw-text-gray-600 tw-transition-all tw-duration-200 tw-rounded-lg tw-whitespace-nowrap ' . $this->getActiveState($item) . '" ' . $item->getAttributes() . '>' .
     ...
     * {@inheritdoc}.
     */
    public function getActiveState($item, $state = '  sidebar-active ')
    {
     ...
     * @return null|string
     */
    public function getActiveStateOnChild($item, $state = 'tw-pb-1 tw-rounded-md  sidebar-active')
    {
   ...
   {
       return '<div class="tw-flex tw-items-center tw-gap-3 tw-px-3 tw-py-2
           tw-text-xs
           tw-font-semibold
           tw-uppercase
           tw-tracking-wider
...
//     {
//         $dropdownToggle = '<a href="#" title="" class="drop_down tw-flex tw-items-center tw-gap-3 tw-px-3 tw-py-2 tw-text-sm tw-font-medium tw-tracking-tight tw-text-gray-600 tw-transition-all tw-duration-200 tw-rounded-lg tw-whitespace-nowrap focus:tw-text-gray-900 sidebar-active' . $this->getActiveStateOnChild($item) . '" ' . $item->getAttributes() . '>' .
//         $this->formatIcon($item->icon) . ' <span class="tw-truncate">' . $item->title . '</span>' .
...
public function getMenuWithDropDownWrapper($item)
{
   ...
   class="drop_down tw-flex tw-items-center tw-gap-3 tw-px-3 tw-py-2
   tw-text-xs tw-font-semibold tw-uppercase tw-tracking-wider
   tw-text-gray-600 tw-transition-all tw-duration-200
   tw-rounded-lg tw-whitespace-nowrap
   '.$this->getActiveStateOnChild($item).'"

    '.$item->getAttributes().'>' .
    $this->formatIcon($item->icon) . '
    <span class="tw-truncate">' . $item->title . '</span>
    <svg aria-hidden="true"
        class="svg tw-ml-auto tw-text-gray-500 tw-size-4 tw-shrink-0"
    ...
    return '<div>' . $dropdownToggle . $this->getChildMenuItems($item) . '</div>' . PHP_EOL;
}
    ...
    public function getChildMenuItems($item)
    {
        ...
        if (count($item->getChilds()) > 0) {
            ...
            foreach ($item->getChilds() as $child) {
                ...
                $children .= '<a href="' . $child->getUrl() . '"
                                 class="submenu-link tw-flex tw-items-center tw-text-sm tw-font-small tw-tracking-tight tw-transition-all tw-duration-200 tw-whitespace-nowrap ' . ($child->isActive() ? 'active' : '') . '"
                                 ' . $child->getAttributes() . '>' .
